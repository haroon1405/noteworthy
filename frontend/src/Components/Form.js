...
export default function Form() {
    ...
    async function handleSubmit(event) {
        ...
        if (!response.ok) {
            let err = json.error
            if(err==='Title and Description is required'){
                setMissData((prev)=>{
                    return{
                        formTitle: true,
                        formDesc: true
            ...
            else if(err==='Description is required'){
                setMissData((prev)=>{
                    return{
                        formTitle: false,
                        formDesc: true
        ...
        <div className="flex items-center justify-center">
            ...
            {isOpen && (
                <div className="fixed inset-0 animate-fade-in flex-col flex items-center justify-end pb-[70px] sm:pb-0 sm:justify-center bg-gray-800 bg-opacity-50">
                    <div className="bg-white rounded-lg p-8 relative w-80 sm:w-[450px]">
                        ...
                        <form className='flex flex-col' onSubmit={handleSubmit}>
                            <input type="text" className={`${missData.formTitle? 'border-red-500' : 'border-gray-200 hover:border-gray-400'} outline-none border-2 mb-2 py-2 mt-5 rounded-lg px-2 text-gray-700`} placeholder='Title' onChange={handleChange} name="formTitle" value={formData.formTitle} />
                            <textarea className={`${missData.formDesc? 'border-red-500' : 'border-gray-200 hover:border-gray-400'} outline-none border-2 mb-2 h-[200px] py-2 rounded-lg px-2  text-gray-700`} placeholder='Description' onChange={handleChange} name="formDesc" value={formData.formDesc} />
